...
def _build_cross_series_resolved_binding_fixture(
    *,
    now: dt.datetime,
...
async def _seed_cross_series_resolved_binding_brief_scenario(
    *,
    ...
    episode, header = _build_brief_test_episode(
    ...
    foreign_document, foreign_revision, cross_series_binding = (
        _build_cross_series_resolved_binding_fixture(
            now=now,
...
class TestEpisodeTemplateService:
    ...
    async def test_build_series_brief_rejects_cross_series_resolved_bindings(
        ...
        async with SqlAlchemyUnitOfWork(session_factory) as uow:
            episode_id = await _seed_cross_series_resolved_binding_brief_scenario(
                uow=uow,
                profile_id=profile.id,
                now=now,
            )
