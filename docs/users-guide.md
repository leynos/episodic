...
- Generated scripts can now receive an internal Chrono spoken-runtime estimate
  during QA. Chrono uses the canonical TEI script, counts spoken words with a
  deterministic local heuristic, and records estimator metadata for audit and
  future comparison. Chrono does not call a Large Language Model (LLM) and does
  not add provider usage charges.
