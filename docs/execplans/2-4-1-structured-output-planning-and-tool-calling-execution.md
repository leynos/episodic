...
2. An application-service execution layer that interprets the typed plan and
   ...
   actions.
3. A small LangGraph wrapper that executes the sequence `initialize -> plan ->
   execute ->
   finish`, using typed state and conditional edges where useful. For `2.4.1
   `, it remains in-process.
